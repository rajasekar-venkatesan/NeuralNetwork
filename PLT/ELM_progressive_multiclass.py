...
if __name__ == '__main__':
    ...
    # train_X, test_X, train_y, test_y = train_test_split(feats, labels_raw, test_size=0.2, random_state=42)
    # print('Divided into training and testing set')

    test_samples = 20
    train_samples = len(feats) - test_samples
    train_X = feats[:train_samples]
    test_X = feats[train_samples:]
    train_y = labels_raw[:train_samples]
    test_y = labels_raw[train_samples:]
    ...
    labels2index_map = {str(label): ind for ind, label in enumerate(labels_set)}
    print('Labels to index map: {}'.format(labels2index_map))
    y0 = np.array([labels2index_map[str(label)] for label in y0])
    ...
    beta = beta0
    ##Subsequent Block
    for X, y in zip(train_X[N0:], train_y[:N0]):
        X = X.reshape((1, 4))
        y = to_bipolar([labels2index_map[str(y)]], labels2index_map).reshape(1, 3)
        # y = y.reshape((1, 3))
